...
## 1. 개요 (Overview)
TubeTalk 프로젝트의 Python 환경을 **Poetry** 기반으로 관리하고, **PoeThePoet (poe)**를 적용하여 단일 명령어로 포맷팅, 린팅, 정적 분석 및 90% 커버리지 유닛 테스트를 수행하는 품질 관리 체계를 구축합니다.
...
* **Poe Tasks (`[tool.poe.tasks]`)**:
  - `test`: `pytest --cov=tubetalk --cov-report=term-missing --cov-fail-under=90`
  - `format`: `ruff format .`
  - `lint`: `ruff check . && mypy tubetalk`
  - `check`: `["format-check", "lint", "test"]` (포맷 검사, 린트/정적분석, 유닛테스트 일괄 수행)
  - `format-check`: `ruff format --check .`

#### 2) Pydantic Settings (`tubetalk/core/config.py`)
* `Settings` 클래스:
  - `GEMINI_API_KEY: str = ""`
...
- [x] **Check 0.2**: `tubetalk/core/config.py` 구현 (Settings & 데이터 경로 자동 생성)
