...
### 2.1 주요 모듈 사양
1. **`jesseagent/core/cache.py` (LocalCacheManager)**:
...
2. **`jesseagent/pipeline/loader.py` (YouTubeLoader)**:
...
- [x] **Check 1.1**: `jesseagent/core/cache.py` 구현 (캐시 저장/로드, `list_cached_videos`, `get_video_status`) 및 유닛 테스트 작성
- [x] **Check 1.2**: `jesseagent/pipeline/loader.py` (URL video_id 파싱 & transcript/metadata 수집) 구현 및 Mock 유닛 테스트 작성
